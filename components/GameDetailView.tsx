...
const GameDetailView: React.FC<GameDetailProps> = ({
    ...
    return (
        <div className="w-full max-w-5xl mx-auto p-4 md:p-8 space-y-8 pb-32">
            ...
            {/* MATCH REPORT VIEW */}
            {type === 'report' && match && (
                <div className="animate-in fade-in slide-in-from-bottom-5 [animation-duration:400ms] space-y-12">
                    {/* Hero Stats */}
                    <div className="relative h-[400px] rounded-[48px] overflow-hidden group">
                        <Image src={match.imageUrl} alt={match.title} fill priority sizes="(max-width: 1024px) 100vw, 1024px" className="object-cover transition-transform duration-700 group-hover:scale-105" />
            ...
            {(type === 'join' || type === 'manage') && game && (
                <div className="animate-in fade-in slide-in-from-bottom-5 [animation-duration:400ms] grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">

                    {/* Left Col: Info & Image */}
                    <div className="lg:col-span-2 space-y-8">
                        <div className="relative aspect-video rounded-[48px] overflow-hidden border border-white/10 shadow-2xl">
                            <Image src={game.imageUrl} alt={game.title} fill priority sizes="(max-width: 1024px) 100vw, 660px" className="object-cover" />
                            ...
                            <div className="flex-1 space-y-4 overflow-y-auto max-h-[400px] pr-2 custom-scrollbar">
                                {game.participants?.map(p => (
                                    <div key={p.id} className="flex items-center gap-4 p-3 hover:bg-white/5 rounded-2xl transition-colors">
                                        <Image src={p.avatar} alt={p.name} width={40} height={40} className="w-10 h-10 rounded-full object-cover border border-white/10" />
